...
const allUpperCase = (arr) => {
  // Solution code here...
  let newArr = [];
  arr.forEach(number => newArr.push(number.toUpperCase()));
  return newArr;
};
...
const addValues = (arr, value) => {
  // Solution code here...
  // arr.forEach(number => arr.push(number + value));
  arr.push(value);
};

const addNumbers = (num, arr, times, callback) => {
  // Solution code here...
  // arr.forEach(times (callback(num)));
  for (let i=0; i<times; i++) {
    callback(arr, num);
  }
  // const X = [1,2,3,4,5];
  // X.forEach(() => callback(arr, num));
  return arr;
};
...
const createList = (availableItems) => {
  ...
  availableItems.forEach(item => {
    if (item.available) newList.push(item.name);
  });
  return newList;
};
