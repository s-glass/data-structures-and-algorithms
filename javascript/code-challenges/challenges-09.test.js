...
const maxInArray = (arr) => {
  // Solution code here...
  return.arr.reduce(function(maximum, current) {
    if (current > maximum) {
      return current;
    } else {
      return maximum;
...
const updateNumbers = (obj) => {
  // Solution code here...
  let newData = [];
  Object.keys(pnj).forEach(key => newData.push(`${key}: ${obj[key]}`));
...
const hasChildrenValues = (arr, character) => {
  ...
  arr.forEach(person => {
    ...
    values.length === 4 ? hasChildren = true : hasChildren; // 4 is 4 values
  }
  });
  return hasChildren;
